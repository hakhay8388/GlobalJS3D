...
var cSpotLigth = Class(cBaseLight,
{
    ObjectType: ObjectTypes.cSpotLigth
    , SpotLigthModel: null
    , LightSpotAngle : (Math.PI / 2)
    , SmoothAngle : (Math.PI / 2)
    ,
    constructor: function (_OwnerScene, _X, _Y, _Z, _AimX, _AimY, _AimZ, _LightColor, _Intensity, _LightSpotAngle, _SmoothAngle)
    {
        cSpotLigth.BaseObject.constructor.call(this, _OwnerScene, _X, _Y, _Z, _AimX, _AimY, _AimZ, _LightColor, _Intensity);
        this.LightSpotAngle = _LightSpotAngle;
        this.SmoothAngle = _SmoothAngle;        
        var __Rotate = this.GetRotate();
        var __CoreMaterial = new cSolidColor(_OwnerScene, Colors.White, 100, 100);
        var __Material = new cLambert(_OwnerScene, __CoreMaterial, 0, 1,  Colors.White, 0, 4, new cColor(0.8, 0.8, 0.8, 1), 1, false, Colors.White);
        this.SpotLigthModel = _OwnerScene.CreateModel(_X, _Y, _Z, -(__Rotate.RotateX - (Math.PI / 2)), __Rotate.RotateY, __Rotate.RotateZ, 1, 1, 1, new cSpotLightMesh(), __Material, -1, true, false, false, false);
    ...
    ,
    SetAim : function(_X, _Y, _Z)
    {
        this.AimX.Value(_X);
        this.AimY.Value(_Y);
        this.AimZ.Value(_Z);
        var __Rotate = this.GetRotate();
        this.SpotLigthModel.Rotate(-(__Rotate.RotateX - (Math.PI / 2)), __Rotate.RotateY ,__Rotate.RotateZ);
        
    }
    ,
    GetRotate: function ()
	{
		var __RotateX = cMath.SlopeAngle3d(this.AimX.Value(), this.AimY.Value(), this.AimZ.Value(), this.X.Value(), this.Y.Value(), this.Z.Value());
		var __RotateY = cMath.NormalizeAngle(cMath.DirectionedAngle(this.X.Value(), this.Z.Value(), this.AimX.Value(), this.AimZ.Value(), this.AimX.Value(), this.AimZ.Value() + 10));
    ...
    Render: function (_Camera, _Material, _LightNo, _Model, _TextureData, _OrgTextureData, _FaceCenter, _Vertex1, _Vertex2, _Vertex3, _SurfaceNormalVector, _uv_X1, _uv_Y1, _Vertex1NormalVector, _uv_X2, _uv_Y2, _Vertex2NormalVector, _uv_X3, _uv_Y3, _Vertex3NormalVector)
    {
        ...
        var __CenterVector = new cVector3d(this.X.Value() - this.AimX.Value(), this.Y.Value() - this.AimY.Value(), this.Z.Value() - this.AimZ.Value());
        ...
        for (var x = __MinForX; x < __MaxForX; x++)
        {
            for (var y = __MinForY; y < __MaxForY; y++)
            {
               ...
               var __Angle = cMath.AngleBetween(__LightVector, __CenterVector) 
               
               var __Dot = 0;
               if (__Angle < this.LightSpotAngle)
               {
                    __Dot = this.Saturate(__LightVector.Dot(__SurfaceVector));
               }
               else if (this.LightSpotAngle < __Angle &&  __Angle < this.SmoothAngle)
               {
                    __Dot = this.Saturate(__LightVector.Dot(__SurfaceVector));
                    var __Diffrence = this.SmoothAngle - __Angle;
                    var __FullDiffrence = this.SmoothAngle - this.LightSpotAngle;
                    __Dot = (1 - (__Diffrence / __FullDiffrence)) * __Dot;
               }
               else
               {
                    __Dot = 0;
