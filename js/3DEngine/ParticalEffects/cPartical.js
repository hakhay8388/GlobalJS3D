...
var cPartical = Class(cBasePartical, {
	ObjectType: ObjectTypes.cPartical
	, OrgSpeedX : 0
	, OrgSpeedY : 0
	, OrgSpeedZ : 0
	, SpeedX : 0
	...
	{
		cPartical.BaseObject.constructor.call(this, _OwnerEmiter, _Model);

		this.UseRandomScale();
		this.UseRandomRotate();
		this.UseRandomParticalRotateVelocity();
	...
	ControlLifeSpanFinished: function ()
	{
		...
		if (this.OwnerEmiter.LifeSpan != 0)
		{
		    ...
		    {
		        var __Scale = 0;
		        if (this.OwnerEmiter.LifeSpanStartScale !=0)
		        {
			         __Scale = (((this.OwnerEmiter.LifeSpanFinishScale - this.OwnerEmiter.LifeSpanStartScale) * ((__Now - this.CreateTime) / this.OwnerEmiter.LifeSpan)) + this.OwnerEmiter.LifeSpanStartScale);
			         this.Model.Scale(__Scale, __Scale, __Scale);
			         return false;
			    }
			    else
			    {
				     __Scale = (((this.OwnerEmiter.LifeSpanFinishScale - this.StartScale) * ((__Now - this.CreateTime) / this.OwnerEmiter.LifeSpan)) + this.StartScale);
			         this.Model.Scale(__Scale, __Scale, __Scale);
			         return false;			
			    }
		    }
        ...
        {
            var __Scale = ((this.OwnerEmiter.LifeSpanFinishScale - this.StartScale) + this.StartScale);
            this.Model.Scale(__Scale, __Scale, __Scale);
            return false;			
        }
		return false;
	...
	UseRandomParticalVelocity : function()
	{
        ...
        this.Model.Translate(__X , __Y, __Z);

	}
	,
	UseRandomScale : function()
	{
	    var __MinScale = this.OwnerEmiter.MinRandomParticalScale;
	    var __MaxScale = this.OwnerEmiter.MaxRandomParticalScale;
	    
	    if (this.OwnerEmiter.UseRandomParticalScale)
	    {
	        var __Scale = __MinScale + ((__MaxScale - __MinScale) * Math.random());
	        this.StartScale = __Scale;
	        this.Model.Scale(__Scale, __Scale, __Scale);
	    }
	}
	,
	UseRandomRotate : function()
	{
        if (this.OwnerEmiter.UseRandomParticalRotate)
	    {
	        this.Model.Rotate((Math.random() * (Math.PI * 4)) - (Math.PI * 2), (Math.random() * (Math.PI * 4)) - (Math.PI * 2), (Math.random() * (Math.PI * 4)) - (Math.PI * 2));
	    }
	    else
	    {
	        this.Model.Rotate(this.OwnerEmiter.ParticalRotate.X, this.OwnerEmiter.ParticalRotate.Y, this.OwnerEmiter.ParticalRotate.Z);
	    }
	}	
	,
	UseRandomParticalRotateVelocity : function()
	{
	    if (this.OwnerEmiter.UseRandomParticalRotateVelocity)
	    {
	        this.Model.RotateVelocityVector.X = Math.random() * 0.01;
	        this.Model.RotateVelocityVector.Y = Math.random() * 0.01;
	        this.Model.RotateVelocityVector.Z = Math.random() * 0.01;
	    }
	    else
	    {
	        this.OwnerEmiter.ParticalRotateVelocity.Normalize();
	        this.Model.RotateVelocityVector.X = this.OwnerEmiter.ParticalRotateVelocity.X;
	        this.Model.RotateVelocityVector.Y = this.OwnerEmiter.ParticalRotateVelocity.Y;
	        this.Model.RotateVelocityVector.Z = this.OwnerEmiter.ParticalRotateVelocity.Z;
	    }
	}
	,
	BaseObject: function ()
